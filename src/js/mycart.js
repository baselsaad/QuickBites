  ...
  var loadMyCartEvent = function (userOptions) {
    ...
    var drawTable = function () {
      ...
      $.each(products, function () {
        ...
        var correctImage = document.querySelectorAll('.product-img')[this.id - 1];
        var correctImageUrl = correctImage.getAttribute('src');

        $cartTable.append(
          '<tr title="' + this.summary + '" data-id="' + this.id + '" data-price="' + this.price + '">' +
          '<td class="text-center"><img style="width:60px" src="' + correctImageUrl + '"/></td>' +
          '<td>' + this.name + '</td>' +
    ...
    */
    ...
    $("#" + idCheckoutCart).click(async function () {
      var products = ProductManager.getAllProducts();
      if (!products.length) {
      ...
      try {
        const response = await fetch('http://localhost:8081/v1/add/cart', {
          method: 'POST',
